...
export const Login: React.FC = () => {
  ...
  // Wyciągnięty z .env URL przekierowania (lub fallback na origin strony)
  const redirectTo =
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        setMessage('✅ Konto utworzone! Sprawdź skrzynkę email lub zaloguj się.');
        setIsSignUp(false);
      } else {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      const { error: oauthError } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          // Po udanym logowaniu Supabase przekieruje użytkownika pod adres z .env
          redirectTo,
        },
      });
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-950 p-4 text-slate-100">
      <div className="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900/80 p-8 shadow-2xl backdrop-blur-xl">
        ...
        {/* Oficjalny styl przycisku Google */}
        <button
          type="button"
          onClick={handleGoogleLogin}
          disabled={loading}
          className="flex w-full items-center justify-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-800 shadow-sm transition-all hover:bg-slate-50 active:scale-[0.98] disabled:opacity-50"
        >
          {loading ? (
          ...
          <span>{loading ? 'Przekierowywanie do Google...' : 'Zaloguj się przez Google'}</span>
        </button>

        <div className="my-6 flex items-center gap-3">
          <div className="h-px flex-1 bg-slate-800" />
          <span className="text-xs font-medium uppercase text-slate-500">lub przez email</span>
          <div className="h-px flex-1 bg-slate-800" />
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-xl border border-slate-700 bg-slate-800/50 py-3 pl-10 pr-4 text-sm text-white placeholder-slate-500 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
              />
            </div>
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 py-3 text-sm font-semibold text-white shadow-lg shadow-indigo-600/30 transition-all hover:bg-indigo-500 active:scale-[0.98] disabled:opacity-50"
          >
            {loading ? (
